...
export class CurioBoxService {
    ...
    async create(createCurioBoxDto: CreateCurioBoxDto): Promise<CurioBox> {
        ...
        if (itemIds.length > 0 || itemProbabilities.length > 0) {
            ...
            // 查出 items 实体数组
            const dbItems = await this.itemRepository.find({
                where: { id: In(itemIds) },
            });
    ...
    async update(
        id: number,
        updateCurioBoxDto: UpdateCurioBoxDto,
    ): Promise<CurioBox> {
        // 先查出当前 curioBox
        const oldCurioBox = await this.curioBoxRepository.findOne({
            where: { id },
            relations: ['items'],
        });
        if (!oldCurioBox) {
        ...
        const itemIds =
            updateCurioBoxDto.itemIds ??
            oldCurioBox.items?.map((i) => i.id) ??
            [];
        const itemProbabilities =
            updateCurioBoxDto.itemProbabilities ??
            oldCurioBox.itemProbabilities;
        await this.validateItemProbabilities(itemProbabilities, itemIds);
        // 查出 items 实体数组
        const dbItems = await this.itemRepository.find({
            where: { id: In(itemIds) },
    ...
    // 新增：修改items和概率列表的方法
    async updateItemsAndProbabilities(
        id: number,
        itemIds: number[],
        itemProbabilities: { itemId: number; probability: number }[],
    ): Promise<CurioBox> {
    ...
    async remove(id: number): Promise<void> {
        ...
        if (curioBox.items && curioBox.items.length > 0) {
            for (const item of curioBox.items) {
                ...
                if (fullItem) {
                    fullItem.curioBoxes =
                        fullItem.curioBoxes?.filter((cb) => cb.id !== id) || [];
