...
const HistoryPage: React.FC = () => {
    ...
    return (
        <Box>
            <Typography variant="h4" gutterBottom>开箱记录</Typography>
                                ...
                                >
                                    <TableCell component="th" scope="row">
                                        <Box sx={{ display: 'flex', alignItems: 'center' }}>
                                            <Avatar src={box.item?.image} sx={{ mr: 2 }} variant="rounded" />
                                            {box.item?.name || '未知物品'}
                                        </Box>
                                    </TableCell>
                                    <TableCell align="left">{box.curioBox.name}</TableCell>
                                    <TableCell align="center">
                                        {box.item && <RarityChip rarity={box.item.rarity} />}
                                    </TableCell>
                                    <TableCell align="right">
                                        {new Date(box.purchaseDate).toLocaleDateString()}
                                    </TableCell>
                                </TableRow>
                            ))}
                        </TableBody>
