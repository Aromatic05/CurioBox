...
const RarityChip: React.FC<{ rarity: IItem['rarity'] }> = ({ rarity }) => {
    const colorMap: Record<string, 'default' | 'primary' | 'secondary' | 'error'> = {
        common: 'default',
        rare: 'primary',
...
const CurioBoxDetailPage: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const fetchAllItems = async () => {
            ...
            const promises = ids.map(id => fetchItemById(id).then(res => res.data).catch(() => undefined));
    ...
    const handlePurchase = async () => {
        ...
        try {
            ...
            const errorMessage = err.response?.data?.message || '购买失败，库存可能不足。';
    ...
    if (error || !box) return <Alert severity="error" sx={{ mt: 4 }}>{error || '未找到该盲盒。'}</Alert>;
    ...
    return (
        <Container maxWidth="lg" sx={{ my: 4 }}>
            <Paper elevation={3}>
                <Box sx={{ display: 'flex', flexDirection: { xs: 'column', md: 'row' } }}>
                    <Box sx={{ flex: 1 }}>
                        <Box
                            component="img"
                            src={box.coverImage || `https://via.placeholder.com/600x400?text=${box.name}`}
                            alt={box.name}
                    ...
                    <Box sx={{ flex: 1, p: 4, display: 'flex', flexDirection: 'column' }}>
                        ...
                        <Box sx={{ mt: 'auto' }}>
                            <Button
                                variant="contained"
                                size="large"
                                startIcon={purchaseLoading ? <CircularProgress size={20} color="inherit" /> : <ShoppingCartIcon />}
                                onClick={handlePurchase}
                                disabled={!isAuthenticated || purchaseLoading}
                            >
                                {purchaseLoading ? '处理中...' : '立即购买'}
                            </Button>
                            {!isAuthenticated && <Typography variant="caption" color="error" sx={{ display: 'block', mt: 1 }}>请先登录再进行购买</Typography>}
                        </Box>
                    </Box>
                </Box>
            </Paper>
            ...
            <Box sx={{ mt: 5 }}>
                <Typography variant="h4" gutterBottom>盲盒内容物一览</Typography>
                <List>
                        ...
                        if (!item) {
                            return (
                                <ListItem key={prob.itemId}>
                                    <ListItemText primary={`未找到 itemId: ${prob.itemId}`} secondary={JSON.stringify(prob)} />
                                </ListItem>
                            );
                        }
                        return (
                            <React.Fragment key={item.id}>
                                <ListItem>
                                    <ListItemAvatar>
                                        <Avatar variant="rounded" src={item.image || 'https://via.placeholder.com/56'} sx={{ width: 56, height: 56, mr: 2 }} />
                                    </ListItemAvatar>
                                    <ListItemText
                                        primary={item.name}
