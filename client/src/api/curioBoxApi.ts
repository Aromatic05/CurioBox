...
// 获取单个盲盒详情
export const getCurioBoxById = (id: number): Promise<AxiosResponse<ICurioBox>> => {
...
// 批量更新盲盒物品及概率
export const updateCurioBoxItemsAndProbabilities = (id: number, payload: { itemIds?: number[]; itemProbabilities?: { itemId: number; probability: number }[] }): Promise<AxiosResponse<ICurioBox>> => {
    return apiClient.patch(`/curio-boxes/${id}/items-and-probabilities`, payload);
};

// 搜索盲盒
export const searchCurioBoxes = (q: string): Promise<AxiosResponse<ICurioBox[]>> => {
    return apiClient.get(`/curio-boxes/search?q=${encodeURIComponent(q)}`);
};

// 创建带图片的盲盒（multipart/form-data）
export const createCurioBoxWithCover = (formData: FormData): Promise<AxiosResponse<ICurioBox>> => {
